...
import { analyzeSheet, buildDashboard, type Analysis } from '@/lib/analyze'
...
import { EmptyState, LoadingState, ErrorState, DashboardGenerating } from './StateViews'
...
export function AppShell({ onLogout }: Props) {
  ...
  const handleFileLoaded = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        handleLoaded(parsed.sheets, parsed.fileName)
      } catch (err) {
        setImportError(
          err instanceof Error ? err.message : 'Não foi possível processar este arquivo.',
        )
      } finally {
        setImporting(false)
      }
  ...
  const duplicateComponent = useCallback(
    (id: string) => {
      ...
      const nextLayout = [...workbook.layout]
      // posiciona logo abaixo
      const maxY = nextLayout.reduce((m, l) => Math.max(m, l.y + l.h), 0)
      nextLayout.push({ i: newId, x: 0, y: maxY, w: layoutItem?.w ?? 6, h: layoutItem?.h ?? 3 })
      setWb({ ...workbook, dashboard: [...workbook.dashboard, copy], layout: nextLayout })
    },
    [workbook, setWb],
  )
  ...
  const addComponent = useCallback(() => {
    ...
    const comp: DashComponent = {
      ...
      columnX: activeSheet?.columns[0],
      columnY: activeSheet?.columns.find(
        (c) => workbook.analysis?.hasRevenue && c.toLowerCase().includes('receita'),
      ),
      format: 'currency',
      labels: [],
      values: [],
  ...
  const buildAgentContext = useCallback((): { data: string; dash: string } => {
    ...
    const dashLines = workbook.dashboard.map((c) => `${c.type}:${c.title}`)
    return { data: lines.join('\n'), dash: dashLines.join('\n') }
  }, [workbook, activeSheet])

  const executeAgentAction = useCallback(
    (result: AgentResult): string => {
      // aplica a ação real ao estado
      switch (result.action) {
        case 'add_chart': {
          ...
          const colX =
            typeof result.params.columnX === 'string'
              ? result.params.columnX
              : activeSheet?.columns[0]
          const colY = typeof result.params.columnY === 'string' ? result.params.columnY : undefined
          const size = typeof result.params.size === 'string' ? result.params.size : 'medium'
          if (!workbook) return 'Sem dados para criar gráfico.'
          // calcula dados reais para o gráfico
          const data = computeChartData(activeSheet, colX, colY, chartType)
          const comp: DashComponent = {
            id: `cmp-${Date.now()}`,
            type: chartType,
            title,
    ...
    [workbook, activeSheet, setWb, moveComponent, createSheet],
  )
  ...
  const generateDashboard = useCallback(() => {
    ...
    setGenerating(true)
    setTimeout(() => {
      const a = analyzeSheet(activeSheet ?? workbook.sheets[0])
      const { components, layout } = buildDashboard(activeSheet ?? workbook.sheets[0], a)
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      {/* Modal de importação (quando já há arquivo) */}
      {showImport && workbook && (
        <div
      ...
      <AgentPanel
        ...
        state={agentState}
        onStopListening={() => {
          // para o reconhecimento no painel
        }}
      />
    </div>
  )
...
// helper: calcula dados para gráfico a partir da aba ativa
function computeChartData(
  sheet: Sheet | null,
  colX?: string,
  colY?: string,
  chartType?: DashComponent['type'],
): { labels: string[]; values: number[]; rows?: { label: string; value: number }[] } {
  if (!sheet) return { labels: [], values: [] }
  const xi = colX
    ? sheet.columns.findIndex((c) => c.toLowerCase().includes(colX.toLowerCase()))
    : -1
  const yi = colY
    ? sheet.columns.findIndex((c) => c.toLowerCase().includes(colY.toLowerCase()))
    : -1

  if (chartType === 'pie' || chartType === 'bar' || chartType === 'ranking') {
    if (xi >= 0 && yi >= 0) {
