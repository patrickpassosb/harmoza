// HARMOZA — grade modular arrastável do dashboard
      ...
      <div className="flex flex-col items-center justify-center rounded-2xl border border-border bg-card p-16 text-center animate-fade-in">
        <div className="grid grid-cols-3 gap-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="h-20 w-24 animate-pulse-soft rounded-xl bg-muted"
              style={{ animationDelay: `${i * 120}ms` }}
            />
          ))}
        </div>
        <p className="mt-6 font-semibold text-foreground">Gerando seu dashboard…</p>
  ...
  return (
    <div className={dragging ? 'cursor-grabbing' : ''}>
      <Grid
        ...
        onLayoutChange={(next) => {
          moveComponent(next as typeof layout)
        }}
      ...
      >
        {components.map((comp) => (
          ...
          >
            ...
            <div className="absolute right-1 bottom-1 h-3 w-3 cursor-se-resize rounded-sm opacity-0 transition-opacity group-hover:opacity-100" />
          </div>
        ))}
      </Grid>
    </div>
