...
import { Bot, Table2, LayoutDashboard, Cloud, CloudOff } from 'lucide-react'
  ...
  return (
    <header className="flex h-14 shrink-0 items-center justify-between gap-4 border-b border-border bg-card px-5">
        ...
        <div className="flex min-w-0 flex-col">
          ...
          <p className="flex items-center gap-1 truncate text-xs text-muted-foreground">
            {workbook && activeSheet ? (
              <>
                <span className="text-harmoza-teal">●</span>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="mr-1 hidden items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-medium text-emerald-700 sm:flex">
          <Cloud className="h-3 w-3" />
          Salvo na sessão
        </div>

        {/* Alternar planilha / dashboard */}
        {workbook && (
